...
package com.haoyayi.thor.repository.${model};
...
public class ${Model}Repository extends AbstractModelRepository<${Model}Type, ${Model}TypeField, ${Model}ConditionField> {
    ...
    protected Map<Long, Map<${Model}TypeField, Object>> getModelFieldFromDB(Set<Long> ids, Set<${Model}TypeField> fields) {
        ...
        // 字典类查询
        refreshDict(fields4Query, result);
    ...
    public Map<Long, Map<${Model}TypeField, Object>> getModelByCondition(Long optid, List<ConditionPair<${Model}ConditionField>> conditions, Option[] options, Set<${Model}TypeField> fields) {
        ...
        Set<${Mainbo}Column> boFields = EnumUtils.filterAndConvertEnumItem(fields, ${Mainbo}Column.class);
        ...
        Map<Long, Map<${Model}TypeField, Object>> result = new HashMap<Long, Map<${Model}TypeField, Object>>();
        result = MergeUtils.mergeEnumItems(result, ${Model}TypeField.class, ${mainbo}Map);
        // 字典类查询
        refreshDict(fields, result);
        return result;
    }
    ...
    @Override
    public List<Map<String, Object>> getModelGroupByByCondition(Long optid, Map<${Model}TypeField, Object> conditions, Set<${Model}TypeField> groupByFields, Map<GroupFunc, ${Model}TypeField> groupFuncMap) {
        Map<${Mainbo}Column, Object> ${mainbo}Condtions = EnumUtils.filterAndConvertEnumItemMap(conditions, ${Mainbo}Column.class);
        Set<${Mainbo}Column> ${mainbo}GroupByFields = EnumUtils.filterAndConvertEnumItemSet(groupByFields, ${Mainbo}Column.class);
        Map<GroupFunc, ${Mainbo}Column> ${mainbo}GroupFuncMap = EnumUtils.filterAndConvertEnumItemMapValue(groupFuncMap, ${Mainbo}Column.class);
        return ${mainbo}DAO.get${Mainbo}GroupResultByCondition(${mainbo}Condtions, ${mainbo}GroupByFields, ${mainbo}GroupFuncMap);
    }
